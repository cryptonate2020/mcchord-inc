...
This repo now holds a **clean static recreation** of [mcchordinc.com](https://www.mcchordinc.com): Home, Services, Topics, About, Contact, Testimonials, plus a speaker one-sheet. Stub/orphan Wix pages were left unpublished on purpose. GitHub Pages serves the files from `main` `/` (no build step).
...
2. Topic cards with audience takeaways (request-this-session links already land on Contact/Calendly).
