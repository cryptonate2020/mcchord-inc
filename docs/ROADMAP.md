...
3. **Speaker reel near the hero.** Uses the real YouTube overview video behind a click-to-play facade (poster + play button; iframe loads on click). To swap it later: replace the video ID in `data-youtube` and the facade `href` (`index.html`, `#speaker-reel`; also `media.html`) and the matching `VideoObject` URLs. Example: `https://www.youtube-nocookie.com/embed/YOUR_ID`.
