...
3. **Speaker reel in the hero.** The official YouTube overview (`d74fKmOc7TY`) is a click-to-play facade in the homepage hero (local poster + play button; iframe loads on click). Click loads a stripped `youtube-nocookie` embed (`modestbranding=1`, `rel=0`, `iv_load_policy=3`, `playsinline=1`, `disablekb=1`, `controls=1`, `fs=0`, `autoplay=1`). To swap it later: change `data-youtube` and the facade `href` on `.hero-video` in `index.html` (and the matching `VideoObject` URLs / media kit facade). The lower homepage block is a short link to the hero, one-sheet, and media kit — not a second player.
